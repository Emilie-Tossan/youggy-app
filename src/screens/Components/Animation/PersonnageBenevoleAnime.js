...
const PersonnageBenevole2 = props => {
	const wingRight = 45;
	const wingLeft = -wingRight;
	const [ flyLeft, setFlyLeft ] = useState(new Animated.Value(wingLeft));
	const [ flyRight, setFlyRight ] = useState(new Animated.Value(wingRight));
	...
	const [ pivotX, pivotY ] = [ 424.57 / 2, 426.838 / 2 ];
	const offsetAndroid = Platform.OS === 'android' ? width / 2 : 0;
	const flyingRotate = 45;
	const flyingDuration = 500;

	const flying = (val, duration) => {
		let valLeft = wingLeft;
		let valRight = wingRight;
		let translation = 0;
		const nextVal = val ? 0 : flyingRotate;
		if (val === flyingRotate) {
			valLeft = 0;
			valRight = 0;
			translation = 40;
	...
	useEffect(() => {
		flying(flyingRotate, flyingDuration);
		flyingY(flyingRotate, flyingDuration);
	}, []);
	...
	return (
		<AnimatedSvg
			width={width}
			height={height}
			viewBox={`-${pivotX} -${pivotY} 424.57 426.838`}
	...
	c2.188-6.826,4.563-30.543-16.352-42.799C230.686,66.894,195.594,66.451,182.103,73.999z"
			...
			<AnimatedG
				...
				style={{
					transform: [
						{ translateX: -offsetAndroid - 5 },
						{
							rotate: flyRight.interpolate({
								inputRange: [
									wingRight - wingRight,
									wingRight
								],
								outputRange: [
									`${wingRight - wingRight}deg`,
									`${wingRight}deg`
								]
							})
						},
			...
			>
				<AnimatedPath
					...
					d="M239.914,215.521c14.422-22.287,28.012-43.386,42.413-65.686c0.773-1.2,1.451-1.594,2.914-1.225
		c9.848,2.492,19.614,2.063,29.231-1.192c3.712-1.257,7.368-2.67,11.058-3.991c0.665-0.238,1.365-0.379,2.05-0.564l-0.05-0.049
		c-0.03,12.382-3.451,23.258-13.956,30.876c-4.143,3.004-8.894,4.331-13.393,4.398c10.038,2.273,20.221-0.335,30.376-3.216
		c0.065,0.123,0.139,0.248,0.209,0.37c-0.523,0.791-1.031,1.595-1.576,2.371c-4.635,6.609-10.094,12.333-17.506,15.85
		c-11.604,5.502-22.533,3.875-32.894-3.276c-0.861-0.596-1.688-1.236-2.918-2.143c3.504,5.302,7.724,9.242,12.906,11.897
		c5.089,2.606,10.461,4.661,15.683,6.945c-6.68,6.855-23.689,12.402-39.358-1.475c1.078,1.653,2.071,3.37,3.265,4.938
		c1.201,1.58,2.54,3.066,3.919,4.5c1.417,1.473,2.942,2.842,4.444,4.229c1.44,1.342,2.915,2.646,4.523,4.104
		c-4.566,2.334-9.078,3.154-13.879,2.359c-4.676-0.771-8.767-2.779-12.656-5.64c1.433,3.316,2.863,6.629,4.486,10.39
		C252.732,228.454,244.289,220.015,239.914,215.521z"
				/>
			</AnimatedG>
			<AnimatedG
				opacity={0.8}
				...
				style={{
					transform: [
						...
						{
							rotate: flyLeft.interpolate({
								inputRange: [ wingLeft, wingLeft - wingLeft ],
								outputRange: [
									`${wingLeft}deg`,
									`${wingLeft - wingLeft}deg`
								]
							})
						},
						{ translateX: offsetAndroid },
			...
			>
				<AnimatedPath
					transform={`translate(-${pivotX} -${pivotY})`}
					d="M190.143,217.714c-2.47,2.182-6.104,6.057-11.271,10.432c-2.402,1.951-5.123,3.388-8.534,4.371
		c1.604-3.729,3.062-7.123,4.521-10.514c-8.163,6.168-16.729,8.123-26.554,3.471c5.737-5.596,12.187-10.318,15.737-17.668
		c-12.729,10.396-25.716,11.53-39.042,1.688c-0.031-0.197-0.062-0.396-0.098-0.6c1.886-0.723,3.753-1.485,5.656-2.154
		c7.01-2.464,13.562-5.707,18.808-11.127c1.456-1.501,2.729-3.178,3.908-4.999c-0.362,0.246-0.74,0.478-1.087,0.745
		c-5.456,4.235-11.573,6.895-18.479,7.506c-6.683,0.593-12.787-1.318-18.515-4.625c-7.025-4.055-11.996-10.112-16.578-16.978
		c5.394,1.151,10.475,2.281,15.575,3.31c5.168,1.043,10.34,0.464,15.825-0.191c-9.782-0.513-17.027-4.897-22.062-12.83
		c-4.312-6.797-6.06-14.322-5.926-22.68c2.318,0.933,4.28,1.769,6.271,2.516c8.812,3.307,17.791,5.771,27.344,4.832
		c2.905-0.286,5.812-0.756,8.663-1.396c1.355-0.305,1.97,0.058,2.671,1.129c10.14,15.501,20.315,30.975,30.482,46.456
		C181.69,204.842,185.917,211.278,190.143,217.714z"
				/>
			</AnimatedG>
			<G transform={`translate(-${pivotX} -${pivotY})`}>
				<Path
	...
	c-3.471,0.385-4.049-1.897-4.049-1.897"
		...
		s-4.418,3.242-11.845,2.953C182.799,125.587,177.667,121.92,177.667,121.92z"
					...
					<Path d="M173.166,119.445c0,0,2.072,2.06,4.353,2.493c0.601,0.114,1.057-0.211,2.151-0.827c4.383-2.396,8.952-4.759,16.569-1.711
		...
		c5.357-0.364,11.844,4.812,11.844,4.812S242.832,125.587,235.403,125.876z"
					...
					<Path d="M247.947,119.445c-4.724-2.465-7.096-3.605-11.64-3.605c-8.099,0.385-12.747,6.782-12.747,6.782s3.436-2.264,5.832-3.223
