...
export default class App extends React.Component {
	...
	async componentDidMount() {
		...
		try {
			...
			this.setState({ competenceList, interestList });
		} catch (error) {
			console.log(error);
		}
	}
	...
	handleSuggestion = (text, field) => {
		const { interestList, competenceList } = this.state;
		interestList.sort();
		competenceList.sort();
		let matched = '';
		if (field === 'competence') {
	...
	validationFunc = async () => {
		...
		try {
			...
			const ref = firebase
				...
				.collection('missions')
				.doc(uid);
			await ref.set({ [missionUid]: mission }, { merge: true });
			this.props.navigation.navigate('DateCreation', { missionUid });
		} catch (error) {
			console.log(error);
		}
	};
